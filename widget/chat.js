// Konfiguration
const COMPANY_ID = document.currentScript?.getAttribute("data-company-id") || "planovo";
const BASE_URL = document.currentScript?.getAttribute("data-api-url") || "http://localhost:8000";
const API_URL = `${BASE_URL}/api/companies/${COMPANY_ID}/chat`;
...
async function send() {
  ...
  try {
    ...
    if (data.answer) {
      ...
      // Debug-Info in Console (nur für Entwickler)
      if (data.sources && data.sources.length > 0) {
        console.log("Quellen (intern):", data.sources);
      }
